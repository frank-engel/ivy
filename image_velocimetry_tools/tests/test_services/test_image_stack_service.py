...
class TestCreateImageStack:
    ...
    @patch(
        "image_velocimetry_tools.services.image_stack_service.create_grayscale_image_stack"
    )
    def test_create_image_stack_basic(
    ...
    def test_create_image_stack_with_memory_map(
        ...
        result = image_stack_service.create_image_stack(
            sample_image_paths, map_file_path=map_file, map_file_size_thres=1e6
        )
    ...
    def test_create_image_stack_with_progress_callback(
        ...
        # Call service
        result = image_stack_service.create_image_stack(
            sample_image_paths, progress_callback=progress_callback
        )
        ...
        assert (
            mock_create_stack.call_args[1]["progress_callback"]
            == progress_callback
        )
    ...
    def test_create_image_stack_with_nonexistent_files(
        self, image_stack_service
    ):
...
class TestValidateImageStackParameters:
    ...
    def test_validate_valid_parameters(
        self, image_stack_service, sample_image_paths, tmp_path
    ):
        """Test validation with valid parameters."""
        map_file = str(tmp_path / "stack.dat")
        errors = image_stack_service.validate_image_stack_parameters(
            sample_image_paths, map_file_path=map_file, map_file_size_thres=1e9
        )
        assert len(errors) == 0
    ...
    def test_validate_negative_threshold(
        self, image_stack_service, sample_image_paths
    ):
        """Test validation with negative memory threshold."""
        errors = image_stack_service.validate_image_stack_parameters(
            sample_image_paths, map_file_size_thres=-1
        )
        assert "Memory threshold must be positive" in errors

    def test_validate_nonexistent_map_directory(
        self, image_stack_service, sample_image_paths
    ):
        """Test validation with nonexistent map file directory."""
        map_file = "/nonexistent/directory/stack.dat"
        errors = image_stack_service.validate_image_stack_parameters(
            sample_image_paths, map_file_path=map_file
        )
        assert any(
            "Memory map directory does not exist" in error for error in errors
        )

    def test_validate_reports_max_five_missing_files(
        self, image_stack_service
    ):
        ...
        missing_file_errors = [
            e for e in errors if "Image file not found" in e
        ]
...
class TestEstimateMemoryUsage:
    ...
    @patch(
        "image_velocimetry_tools.services.image_stack_service.estimate_image_stack_memory_usage"
    )
    def test_estimate_memory_grayscale(
        self, mock_estimate, image_stack_service
    ):
        """Test memory estimation for grayscale images."""
        mock_estimate.return_value = 1536000  # 640 * 480 * 5 * 1

        result = image_stack_service.estimate_memory_usage(
            480, 640, 5, num_bands=1
        )
...
class TestShouldUseMemoryMap:
    ...
    def test_should_use_memory_map_below_threshold(
        self, mock_estimate, image_stack_service
    ):
        """Test memory map decision when below threshold."""
        mock_estimate.return_value = 5e8  # 500 MB

        result = image_stack_service.should_use_memory_map(
            480, 640, 100, map_file_size_thres=9e8
        )
    ...
    def test_should_use_memory_map_above_threshold(
        self, mock_estimate, image_stack_service
    ):
        ...
        result = image_stack_service.should_use_memory_map(
            1080, 1920, 500, map_file_size_thres=9e8
        )
    ...
    def test_should_use_memory_map_exactly_at_threshold(
        self, mock_estimate, image_stack_service
    ):
...
class TestGetPreprocessingParameters:
    ...
    def test_get_preprocessing_parameters_with_clahe(
        self, image_stack_service
    ):
    ...
    def test_get_preprocessing_parameters_with_auto_contrast(
        self, image_stack_service
    ):
        """Test getting preprocessing parameters with auto-contrast enabled."""
        params = image_stack_service.get_preprocessing_parameters(
            do_auto_contrast=True, auto_contrast_percent=2
        )

        assert params["do_auto_contrast"] is True
        assert params["auto_contrast_percent"] == 2

    def test_get_preprocessing_parameters_both_enabled(
        self, image_stack_service
    ):
...
class TestValidatePreprocessingParameters:
    ...
    def test_validate_valid_preprocessing_parameters(
        self, image_stack_service
    ):
    ...
    def test_validate_negative_tile_sizes(self, image_stack_service):
        ...
        errors = image_stack_service.validate_preprocessing_parameters(
            clahe_horz_tiles=-1, clahe_vert_tiles=8
        )
    ...
    def test_validate_zero_tile_sizes(self, image_stack_service):
        ...
        errors = image_stack_service.validate_preprocessing_parameters(
            clahe_horz_tiles=0, clahe_vert_tiles=0
        )
        assert "CLAHE tile sizes must be positive" in errors

    def test_validate_auto_contrast_percent_below_range(
        self, image_stack_service
    ):
    ...
    def test_validate_auto_contrast_percent_above_range(
        self, image_stack_service
    ):
    ...
    def test_validate_auto_contrast_percent_at_boundaries(
        self, image_stack_service
    ):
        """Test validation with auto-contrast percentage at valid boundaries."""
        errors_zero = image_stack_service.validate_preprocessing_parameters(
            auto_contrast_percent=0
        )
        errors_hundred = image_stack_service.validate_preprocessing_parameters(
            auto_contrast_percent=100
        )
